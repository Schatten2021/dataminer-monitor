...
#[rocket::post("/ping?<id>")]
pub async fn miner_ping(id: String, miner_states: &rocket::State<Arc<State>>) -> rocket::response::content::RawJson<String> {
    ...
    tokio::spawn(async move {
        ...
        if let Some(timeout_duration) = state.get_timeout_period(&id).await {
            ...
            tokio::spawn(async move {
                ...
                send_email(&state, format!("miner {id} went offline!"), r#"
<h1>The server <code>{id}</code> went offline!</h1>
"#.to_string()).await.expect("failed to send email");
            });
        }
        if went_online {
            send_email(&state, format!("miner {id} went online!"), r#"
<h1>The server <code>{id}</code> is back online! Hooray!</h1>"#.to_string()).await.expect("failed to send email");
    }
    });
...
async fn send_email(state: &Arc<State>, subject: String, body: impl lettre::message::IntoBody + Clone) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for target in lock.iter() {
        let email = lettre::Message::builder()
            ...
            .body(body.clone())?;
        rocket::log::private::info!("sending email: {:?}", email);
        mailer.send(&email)?;
    }
    Ok(())
}